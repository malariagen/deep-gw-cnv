"""
Genome-wide gene CNV caller — version 10.

Replaces the CRR-based logistic sharpness metric (v08/v09) with a segment
coverage fraction derived directly from the HMM segmentation output.

In v09 the band filter asked: "is the sorted CRR profile sharp at the
expected gene/flank boundary?"  This is an indirect proxy that relies on a
curve fit to bin-level copy ratios.  The user asked for the metric to be
based on where HMM segments cover the gene instead.

v10 band filter: for calls with CRR in [crr_amp_threshold, cnv_crr_band_upper),
compute the fraction of gene bins that fall within a CN>=2 HMM segment for
that sample.  If the fraction is below cnv_crr_band_cn2_threshold the call is
demoted to CN=1.  For genuine amplifications the HMM should already assign
CN>=2 to the gene region; for borderline CRR-elevation artefacts the HMM will
have left the gene as CN=1, giving a fraction of 0.

New / changed config keys
--------------------------
cnv_crr_band_cn2_threshold  — min fraction of gene bins in a CN>=2 HMM segment
                              for calls in [crr_amp_threshold, cnv_crr_band_upper).
                              Replaces cnv_crr_band_slope_cutoff from v09.
cnv_crr_band_upper          — unchanged: upper bound of the filtered band
                              (calls with CRR >= cnv_crr_band_upper are admitted
                              unconditionally).

Output column change
--------------------
`logistic_slope`  (v08/v09) → `segment_cn2_fraction`  (v10)
Both genome_wide_amplifications.tsv and gene_calls.tsv are affected.
"""
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    # Band segment filter: calls in [crr_amp_threshold, band_upper) are demoted to CN=1
    # if the fraction of gene bins in a CN>=2 HMM segment is below band_cn2_threshold.
    # Set band_upper=inf or band_cn2_threshold=0 to disable (equivalent to v08 behaviour).
    ...
    for chrom in sorted(genes_by_chrom.keys()):
        ...
        for gene in chrom_genes:
            ...
            for idx in amp_indices:
                ...
                # Band segment filter: demote borderline calls not backed by HMM CN>=2.
                if band_filter_on and crr_all[idx] < band_upper:
                    if not np.isfinite(cn2_frac) or cn2_frac < band_cn2_threshold:
                        continue
                amp_rows.append({
                    "sample_id":            sid,
                    "gene_id":              gene_id,
            ...
            if call_id is not None:
                ...
                for idx in range(n):
                    ...
                    if not ok[idx]:
                        ...
                        # Band segment filter for reference genes.
                        if band_filter_on and crr_val < band_upper:
                            cn = 2 if (frac_val is not None and frac_val >= band_cn2_threshold) else 1
                        else:
                            cn = 2
                    else:
                        cn = 1
