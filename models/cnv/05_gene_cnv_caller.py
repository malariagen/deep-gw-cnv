"""
Gene-level CNV caller — version 05: fractional gene coverage for long genes.

Extends 04 by replacing the strict spanning condition with a fractional coverage
check. Version 04 required a single HMM segment to span the entire gene
(x0 <= g_start AND x1 >= g_end). This rejected cases where the sin-loss VAE
(exp 18) produced crisper, shorter segments that together cover most of the gene
but no single segment fully spans it. The strict condition caused PM2_PM3
FNR to jump from 0.41 (exp 17) to 0.74 (exp 18).

Version 05 instead accumulates coverage from all high-confidence CN>=2 segments
overlapping the gene. If the fraction of gene bins covered meets
cnv_min_gene_coverage_fraction (default 0.50), the gene is called CN=2.
The CRR gate veto still applies: calls below cnv_crr_gate_threshold are downgraded
to CN=1 regardless of coverage.

New config key:
    cnv_min_gene_coverage_fraction — min fraction of gene bins covered by CN>=2
        segments to make an amplification call (long genes only).

Existing keys unchanged (same semantics as v04):
    cnv_crr_gate_threshold
    cnv_crr_amp_threshold
    cnv_min_cn1_proportion
    cnv_min_confidence
    cnv_flank_padding
    cnv_crr_min_bins_fallback
"""
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    for gene in GENES_OF_INTEREST:
        ...
        # Precompute gene bin positions for fractional coverage check (long genes).
        ...
        for idx, sid in enumerate(sample_ids):
                ...
                if len(hc1) > 0:
                    ...
                    if covered.sum() / n_chrom >= min_cn1_proportion:
                        ...
                        if fallback_eligible:
                            # Short gene: get initial cn from spanning segment (unchanged from v04).
                            spanning = chrom_segs[
            ...
            if sanity_ok:
                if fallback_eligible:
                    # Short gene: CRR override unchanged from v04.
                    if cn in (-1, 1) and crr_val is not None and crr_val >= crr_amp_threshold:
                        cn = 2
                else:
                    # Long gene: count the fraction of gene bins covered by any
                    # high-confidence CN>=2 segment that overlaps the gene region.
                    ...
                    if n_gene_bins > 0 and len(gene_segs_amp) > 0:
                        ...
                        if covered_g.sum() / n_gene_bins >= min_gene_coverage_frac:
                            cn = 2
                            # CRR gate veto: downgrade if signal is weak.
