"""
Genome-wide gene CNV caller — version 18.

Extends v16 (noise filter + global CRR rescue) by elevating CRR rescue priority
above the per-chromosome sanity check for reference genes.

Motivation: exp 42 showed that sanity-failed samples (< 55% chromosome CN=1) have
masked CRR well above the rescue threshold (p10 = 1.58–1.62 across all genes).
In v16, the sanity check runs first and assigns CN=-1 before the rescue criterion
is evaluated — so these high-CRR samples are never rescued. Moving rescue before
sanity ensures that any sample with a strong gene/flank copy-ratio elevation is
called as CN=2 regardless of chromosome-level HMM noise.

The noise filter (step 0) still runs first: samples with an extreme number of HMM
transitions are genuinely unreliable even for CRR-based rescue.

Call logic for reference genes (in priority order):
    0. Transitions > cnv_max_transitions                            → CN=-1 (noise filter)
    1. CRR >= cnv_crr_rescue_threshold                             → CN=2 (CRR rescue, now before sanity)
    2. Sanity failed (< 55% CN=1 background)                       → CN=-1
    3. HMM coverage >= cnv_min_gene_coverage_fraction              → CN=2 (HMM primary)
    4. Otherwise                                                   → CN=1
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    crr_rescue_threshold     = cfg.get("cnv_crr_rescue_threshold", None)
    ...
    rescue_msg = (
        f"; CRR rescue >= {crr_rescue_threshold} (applies before sanity check)"
        if crr_rescue_threshold is not None else " (no CRR rescue)"
    )
    noise_msg = (
        f"; noise filter max_transitions={max_transitions}"
        ...
        for gene in chrom_genes:
            ...
            if call_id is not None:
                ...
                for i in range(n):
                    ...
                    elif (crr_rescue_threshold is not None
                          and crr_val is not None
                          and crr_val >= crr_rescue_threshold):
                        # CRR rescue runs before sanity: a strong gene/flank copy-ratio
                        # elevation overrides the sanity check. Exp 42 showed that
                        # sanity-failed samples can have CRR >> 1.55 (masked CRR p10
                        # 1.58–1.62), confirming real signal that the sanity check blocked.
                        cn = 2
                    elif not ok[i]:
                        cn = -1  # sanity failed and CRR not elevated
