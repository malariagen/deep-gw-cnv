"""
Genome-wide gene CNV caller — version 07.

Extends version 06 by adding a logistic-regression sharpness metric to every
amplification call.  After flagging a (sample, gene) pair as CN≥2, the caller
takes all per-bin copy ratios in the local window [gene_start - flank_padding,
gene_end + flank_padding], sorts them, normalises both axes to [0, 1], then
fits a logistic sigmoid:

    y = 1 / (1 + exp(-slope * (x - center)))

The `slope` parameter measures how sharply the sorted-CRR distribution
transitions from a low tier (flanking, CN=1 bins) to a high tier (gene, CN=2
bins).  A large slope indicates a clean, bimodal separation — a reliable CNV
boundary.  A sluggish slope near zero indicates noisy, overlapping distributions.
...
All other calling logic is identical to version 06.
"""
...
def _logistic_sharpness(crr_bins):
    """Fit a logistic sigmoid to sorted bin copy ratios; return the slope parameter.

    The bins are sorted by copy ratio value, x is the normalised rank [0, 1] and
    y is the normalised copy ratio [0, 1].  A clear CNV boundary produces a bimodal
    distribution: the sorted values jump sharply at the transition, yielding a large
    sigmoid slope.  A noisy call produces a smooth CDF and a small slope.
    ...
    def _sigmoid(x, slope, center):
        # Clip to avoid overflow in exp for extreme slope values.
    ...
    for chrom in sorted(genes_by_chrom.keys()):
        ...
        for gene in chrom_genes:
            ...
            for idx in amp_indices:
                ...
                slope           = _logistic_sharpness(finite_bins)
                amp_rows.append({
            ...
            if call_id is not None:
                # Pre-compute sharpness for all samples on this reference gene so
                # the inner loop stays clean.
                slopes = np.full(n, np.nan)
                for idx in range(n):
                    crr_window_bins = cr_chrom[idx, window_mask_l]
                    finite_bins     = crr_window_bins[np.isfinite(crr_window_bins)]
                    slopes[idx]     = _logistic_sharpness(finite_bins)
