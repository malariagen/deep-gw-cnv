"""
Genome-wide gene CNV caller — version 15.

Extends v14 with a CRR rescue fallback for reference genes.

Background: exp 31/32 showed that the HMM-primary criterion misses a large fraction
of true amplifications (93% GCH1, 83% PM2_PM3, 62% MDR1 FNs have segment_cn2_fraction=0
even at st=0.90). FN CRR values are high (p50 1.84–2.66), confirming the signal exists
in coverage. The HMM cannot detect these because gene-scale amplifications (~4–5 bins at
400bp) are overwhelmed in the per-sample HMM fit by the rest of the chromosome; the CN=2
state mean gets anchored to stronger signals elsewhere, leaving 1.5–2.0× copy-ratio
elevations classified as CN=1.

The fix: after HMM-primary evaluation, if a reference gene sample was not called amplified
but its CRR exceeds a rescue threshold, promote it to CN=2. Normal samples have TN CRR
p90 of 1.19–1.24, far below the rescue threshold of 1.60, so false positive risk is low.
...
Call logic for reference genes (in priority order):
    ...
    3. CRR >= cnv_crr_rescue_threshold                               → CN=2 (CRR rescue)
    4. Otherwise                                                     → CN=1

Genome-wide amplifications (non-reference genes) still use HMM-primary only; CRR rescue
is restricted to reference genes where we have validated the threshold from diagnostics.
"""
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    rescue_msg = (
        f"; CRR rescue >= {crr_rescue_threshold} for reference genes"
    ...
    print(
        ...
        f"(confidence >= {min_confidence}){rescue_msg}.",
        flush=True,
    )
            ...
            if call_id is not None:
                ...
                for i in range(n):
                    ...
                    if not ok[i]:
                        ...
                        cn = 2  # CRR rescue: strong gene/flank copy-ratio elevation
                    else:
                        cn = 1
