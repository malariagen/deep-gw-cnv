"""
Genome-wide gene CNV caller — version 12.

Extends v11 with a low-coverage filter on the copy-ratio computation.
In exp 29 (400 bp bins), a small fraction (~2%) of bins per sample have
reconstruction near zero while carrying non-zero read counts.  With the
epsilon denominator (counts / (recons + 1e-6)), each such bin produces a
copy_ratio in the millions and the nanmean of the flank collapses to a huge
value — making the final CRR ≈ 0 for almost every sample.  MDR1 FNR jumped
from 0.01 (exp 27) to 0.88 (exp 29) entirely because of this arithmetic
artifact, not because of any model bias.

Fix: before computing per-gene and per-flank means, NaN-out any bin where
raw counts OR reconstructed counts are below hmm_low_cov_threshold (default 10).
np.nanmean then skips those bins automatically.  The threshold is the same
value already used by the HMM to skip low-coverage positions.

No other logic changes relative to v11.
"""
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    # Reuse the HMM low-coverage threshold: bins where raw counts OR reconstruction
    # fall below this value are excluded from the gene/flank copy-ratio means.
    # This prevents a handful of near-zero reconstruction bins from inflating the
    # flank mean and collapsing CRR to ~0 for the entire sample (the v11 bug).
    ...
    # Defaults make v12 backward-compatible with v11 (single-zone behaviour).
    ...
    # mmap_mode='r': file-backed; avoids loading 11 GB into RAM before processing.
    ...
    # float32 halves memory vs float64.  Low-cov mask computed before the division
    # so we can free the originals early.  In-place division (np.divide out=counts_f)
    # avoids allocating a third full array simultaneously.
    counts_f = counts.astype(np.float32);  del counts;  gc.collect()
    recons_f = recons.astype(np.float32);  del recons;  gc.collect()
    # NaN-out low-coverage bins so they are excluded from nanmean computation.
    # Without this filter, bins where recons ≈ 0 produce copy_ratio in the millions
    # (counts / 1e-6), which dominates the flank mean and collapses CRR to near zero.
    low_cov_mask = (counts_f < low_cov_threshold) | (recons_f < low_cov_threshold)
    recons_f += np.float32(1e-6)                           # in-place; no new array
    np.divide(counts_f, recons_f, out=counts_f)            # in-place; reuses buffer
    copy_ratios = counts_f                                  # rename, no copy
    del recons_f
    ...
    for chrom in sorted(genes_by_chrom.keys()):
        ...
        cr_chrom    = copy_ratios[:, chrom_mask]   # [n_samples, n_chrom_bins]; NaN where low-cov
        ...
        for gene in chrom_genes:
            ...
            # nanmean skips NaN values set by the low-coverage filter above.
