"""
Gene-level CNV caller — version 02: adds CRR fallback for short genes.

Identical to 01 except: when HMM-segment call is not amplification but
gene/flank CRR >= cnv_crr_amp_threshold, the call is overridden to CN=2.
Handles GCH1 (~2 bins) where Viterbi cannot overcome self_transition penalty.

For each gene of interest, determines copy number from pre-computed HMM
segments and computes a copy-ratio relative to the flanking region (CRR).

Reads from cfg:
    cnv_min_cn1_proportion — fraction of a chromosome's bins that must be
                             covered by high-confidence CN=1 segments before
                             making a call (sanity check)
    cnv_min_confidence     — minimum HMM posterior confidence for a segment
                             to count toward the CN=1 sanity check
    cnv_flank_padding      — bp distance outside the gene used to define the
                             flank region for CRR normalisation
    cnv_crr_amp_threshold  — gene/flank CRR above which a non-amp HMM call
                             is overridden to CN=2 (new in version 02)
"""
...
def call_gene_cnv(data, segments, gene, min_cn1_proportion, min_confidence, flank_padding, crr_amp_threshold=1.30):
...
def call_all_genes(data, segments, min_cn1_proportion, min_confidence, flank_padding, crr_amp_threshold=1.30):
