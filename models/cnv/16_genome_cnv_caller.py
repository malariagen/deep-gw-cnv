"""
Genome-wide gene CNV caller — version 16.

Extends v15 (CRR rescue) with a per-sample noise filter based on HMM transition count.

Motivation: exp 32/33 show that the HMM over-segments aggressively (transitions p50 =
270–406 per sample across all chromosomes). A haploid organism with a few CNVs at a time
should have O(10) CN state changes; samples with hundreds or thousands of transitions have
HMM outputs dominated by noise rather than signal.  Calling CNVs on these samples
inflates FPs without recovering FNs (FN CRR is still present; the rescue criterion just
fires on noise too).

A per-sample transition count filter addresses this: samples with total CN transitions
above cnv_max_transitions are marked CN=-1 (too noisy to call) before any other criterion
is evaluated.
...
Genome-wide amplifications (non-reference genes) skip noisy samples in addition to the
