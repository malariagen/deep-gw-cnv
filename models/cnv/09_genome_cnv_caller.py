"""
Genome-wide gene CNV caller — version 09.

Extends version 08 by adding a slope-based band filter for borderline calls.

In v08 all samples with CRR >= crr_amp_threshold are called as amplified, regardless
of logistic_slope.  This admits a band of FPs at CRR just above the threshold (e.g.
1.20–1.25) that have flat CRR profiles (low slope) and are not genuine amplifications.
Analysis of exp 25 showed that FPs in the 1.20–1.25 CRR band have median slope ≈ 1.0
while TPs in the same band have median slope ≈ 1.9–4.1 (gene-dependent), with little
overlap above slope ≈ 1.5.  A threshold raise to 1.25 would eliminate these FPs but
also loses the 30 GCH1 TPs that happen to have CRR 1.20–1.25.  The slope filter keeps
them by requiring slope >= cnv_crr_band_slope_cutoff only for the borderline band.

New config keys
---------------
cnv_crr_band_upper          — upper bound of the slope-filter band (default: inf, i.e.
                              filter applies to all calls above the amp threshold unless
                              overridden).  Typically set to 1.25 so calls with CRR >=
                              1.25 are admitted unconditionally.
cnv_crr_band_slope_cutoff   — minimum logistic_slope for calls in [crr_amp_threshold,
                              cnv_crr_band_upper).  Calls below this slope are demoted
                              to CN=1.  Default: 0.0 (disabled — same as v08).

All other calling logic is identical to version 08.
"""
...
def run_cnv_calls(store_path, out_dir, cfg):
    ...
    # Band slope filter: calls in [crr_amp_threshold, band_upper) are demoted to CN=1
    # if logistic_slope < band_slope_cutoff.  Set band_upper=inf or band_slope_cutoff=0
    # to disable (equivalent to v08 behaviour).
    band_upper       = float(cfg.get("cnv_crr_band_upper", float("inf")))
    band_slope_cutoff = float(cfg.get("cnv_crr_band_slope_cutoff", 0.0))
    band_filter_on   = band_upper > crr_amp_threshold and band_slope_cutoff > 0.0
    ...
    if band_filter_on:
        print(
            f"Band slope filter ON: calls in CRR [{crr_amp_threshold}, {band_upper}) "
            f"require slope >= {band_slope_cutoff}",
            flush=True,
        )
    ...
    for chrom in sorted(genes_by_chrom.keys()):
        ...
        for gene in chrom_genes:
            ...
            for idx in amp_indices:
                ...
                # Band slope filter: demote borderline calls with flat profiles.
                if band_filter_on and crr_all[idx] < band_upper:
                    if not np.isfinite(slope) or slope < band_slope_cutoff:
                        continue
                amp_rows.append({
            ...
            if call_id is not None:
                ...
                for idx in range(n):
                    ...
                    if not ok[idx]:
                        ...
                        # Band slope filter for reference genes.
                        if band_filter_on and crr_val < band_upper:
                            cn = 2 if (slope_val is not None and slope_val >= band_slope_cutoff) else 1
                        else:
                            cn = 2
                    else:
                        cn = 1
