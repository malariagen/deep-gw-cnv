"""
HMM segmentation — version 03: chunked loading to avoid OOM on large datasets.

Identical to 02 except run_hmm_all_samples and _hmm_one_sample no longer load
the full counts/reconstructions matrices into RAM upfront.  Instead each worker
receives file paths + a row index and opens its own mmap, reading only its own
row.  This keeps peak RAM at O(n_jobs × n_bins) rather than O(n_samples × n_bins).

On a (53973, 51986) dataset the old approach tried to allocate ~88 GB; this
version uses <1 GB regardless of dataset size.
"""
    ...
    for chrom in np.unique(chroms):
        ...
        for grp in idx_groups:
            if len(grp) >= n_states * 3:
                contigs.append((chrom, pos_v[grp], cr_v[grp]))
...
def run_hmm_all_samples(store_path, out_dir, cfg):
    ...
    args = [
        (sid, idx, counts_path, recons_path, chroms, starts,
         n_states, self_transition, low_cov_threshold)
