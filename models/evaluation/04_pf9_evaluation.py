"""
Evaluation — version 04: gene calls vs ground truth + HMM segment diagnostics
            + near-miss blindspot diagnostics.

Extends v03 with a NEAR-MISS DIAGNOSTICS section that helps identify why the
caller misses true positives (FNs) and where it is likely to miss them in future.

NEAR-MISS DIAGNOSTICS reads 'segment_cn2_fraction_{gene}' from gene_calls.tsv
(fraction of gene bins covered by elevated-CN HMM segments at the calling
confidence threshold). For each FN sample it shows:

  - Distribution of segment_cn2_fraction (p10–p90): how much gene-body coverage
    did the HMM assign to FN samples? Near-threshold fractions (e.g. 0.35–0.49)
    are tunable with a parameter change. Fractions near 0 indicate an HMM-level
    miss that parameter tuning alone cannot fix.

  - n_fn with fraction >= 0.35 and >= 0.25: explicit recovery counts that tell
    you how many FNs would be rescued by lowering cnv_min_gene_coverage_fraction.

  - n_fn with fraction == 0: complete misses — the HMM assigned no elevated-CN
    coverage to the gene at all; these require HMM or VAE improvement.
...
def run_evaluation(out_dir, cfg):
    """Evaluate gene calls against the Pf8 GATK ground truth, plus segment and
