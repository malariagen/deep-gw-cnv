"""
Evaluation — version 02: compare gene calls against the Pf8 GATK ground truth.

Extends v01 with per-(Year, Population) stratified metrics. Groups with fewer
than eval_min_group_n evaluable samples are suppressed to avoid noisy estimates.

Ground truth source: Pf8 GATK-based pipeline
  https://github.com/malariagen/malariagen-pf8-cnv-calling/tree/master
  using DetermineContigPloidy, GermlineCNVCaller, and PostprocessGermlineCNVCalls.
When results diverge from ground truth, consider what this established pipeline
does that the current model struggles to replicate, and where the model may
already be doing better. Note that the ground truth is imperfect — apparent FPs
may represent real signal the GATK pipeline missed.
...
def run_evaluation(out_dir, cfg):
    """Evaluate gene calls against the Pf8 GATK ground truth.
