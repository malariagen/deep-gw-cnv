#!/bin/bash
# Exp 27: Extended band segment filter — demote CRR [1.20, 1.35) calls not backed by CN>=2 HMM segments.
...
OUT="$REPO_ROOT/data/results/27_skip_st0.90_sin_curriculum_r0.30_genome_wide_band_segcov0.5_ext1.35"
...
cfg_path = "experiments/27/config.yaml"
...
print("Calling genome-wide CNVs (band segment filter: CRR [1.20, 1.35) needs CN>=2 coverage >= 0.5)...", flush=True)
